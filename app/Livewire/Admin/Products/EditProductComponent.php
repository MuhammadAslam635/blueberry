...
class EditProductComponent extends Component implements Forms\Contracts\HasForms
{
                    ...
                    Fieldset::make('Product Size Detail')
                        ->schema([
                            Repeater::make('productDimensions')
                                ->relationship('productDimensions')
                                ->schema([
                                    Select::make('width')
    ...
    public function edit(): void
    {
        try {
            ...
            $this->product->productWeights()->delete();
            foreach ($this->productWeights as $weight) {
                $this->product->productWeights()->create([
            ...
            foreach ($this->productDimensions as $dimen) {
                $this->product->productDimensions()->create([
                    'width' => $dimen['width'],
                ]);
            }
            ...
            $this->success('Updated created');

        } catch (\Exception $e) {
            (new ErrorLogHelper)('updating Product', $e->getMessage());
